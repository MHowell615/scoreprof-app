...
import scoreprof_resources.go_to_setup_leagues_btn
import scoreprof_resources.accept
import scoreprof_resources.ignore
import scoreprof_resources.notif_invite_msg_body
import scoreprof_resources.notif_invited_title
import scoreprof_resources.notif_join_request_title
import scoreprof_resources.notifications
import scoreprof_resources.notification
            ...
            Row(
                ...
                IconButton(onClick = { navController.popBackStack() }) {
                    Icon(
                        ...
                        contentDescription = stringResource(Res.string.close_btn)
                    )
                }
                Text(
            ...
            if (currentNotification != null) {
                item {
                    val title = when (currentNotification.type) {
                        NotificationType.JOIN_REQUEST -> stringResource(Res.string.notif_join_request_title)
                        NotificationType.LEAGUE_INVITE -> stringResource(Res.string.notif_invited_title)
                        else -> stringResource(Res.string.notification)
                    }
                    ...
                    val message = when (currentNotification.type) {
                        ...
                        NotificationType.LEAGUE_INVITE -> stringResource(
                            Res.string.notif_invite_msg_body,
                    ...
                    when (currentNotification.type) {
                        ...
                        NotificationType.LEAGUE_INVITE -> {
                            Button(onClick = {
                                ...
                                Text(stringResource(Res.string.go_to_setup_leagues_btn))
                            }
                        }
                        else -> {
